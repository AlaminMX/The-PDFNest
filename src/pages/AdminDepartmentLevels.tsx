...
export default function AdminDepartmentLevels() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Use the same level logic as the rest of the app
  ...
  const handleApproveCourse = async (courseId: string) => {
    const { error } = await supabase
      .from("courses").update({ status: "approved" } as any).eq("id", courseId);
  ...
  const handleMergeCourse = async () => {
    ...
    await supabase.from("community_uploads")
      .update({ course_id: mergeTargetId }).eq("course_id", mergeSource.id);
      ...
      <main className="container mx-auto px-4 py-6 max-w-3xl space-y-3">
                              ...
                              <div className="flex items-center gap-1.5 shrink-0 flex-wrap justify-end">
                                ...
                                {(course as any).status === "pending" && (
                                  <>
                                    ...
                                    <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-foreground hover:bg-muted" title="Merge into existing" onClick={() => { setMergeSource(course); setMergeTargetId(""); }}>
                          ...
                          {isAddingHere ? (
                            <div className="border border-primary/20 rounded-lg p-3 space-y-3 bg-primary/[0.02]">
                              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                                New course — {LEVEL_LABELS[level] ?? `${level} Level`}
                              </p>
                              ...
                              <div className="flex gap-2">
                                ...
                                <Button size="sm" variant="ghost" className="h-8 text-xs" onClick={resetForm} disabled={saving}>
                                  Cancel
                                </Button>
                              </div>
                            </div>
                            ...
                            >
                              ...
                              Add course to {LEVEL_LABELS[level] ?? `${level} Level`}
                            </button>
                          )}
                        </CardContent>
                      </motion.div>
        ...
        <div className="pt-2">
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            {ALL_LEVELS
              .filter(l => deletedLevels.has(l) || groups.find(g => g.level === l)?.courses.length === 0)
              .map(l => (
                <button
                  key={l}
                  type="button"
          ...
          <div className="bg-background rounded-xl border shadow-xl p-6 w-full max-w-md space-y-4">
            ...
            <div className="space-y-1">
              ...
              >
                ...
                {groups
                  .find(g => g.level === mergeSource.level)?.courses
